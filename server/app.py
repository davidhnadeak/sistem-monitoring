...
def formatted_timestamp(timestamp):
    try:
        ...
        return {"datetime": "Invalid Timestamp", "date": "-", "time": "-"}
...
def get_groundwater_quality():
    try:
        ...
        if not kode_pos:
            logging.warning(
                f"Warning in air_tanah_processing: Missing required parameter (kode_pos)"
            )
            return (
                jsonify(
                    {
        ...
        if not items:
            logging.warning(
                f"Warning in get_groundwater_quality: No records found for postal code: {kode_pos}"
            )
            return (
                jsonify(
                    {
        ...
        if df.empty:
            logging.warning(
                f"Warning in get_groundwater_quality: No valid records found for postal code: {kode_pos}"
            )
            return (
                jsonify(
                    {
        ...
        target_mapping = {0: "Layak Minum", 1: "Tidak Layak Minum"}

        for i, item in enumerate(items):
            # Menambahkan hasil prediksi ke setiap item
            pred_target = target_mapping.get(
                int(prediction_classes[i]), "Tidak Diketahui"
            )
        ...
        logging.info(
            f"Success in air_tanah_processing: Records retrieved and processed successfully"
        )
        return (
            jsonify(
                {
...
def get_kode_pos():
    try:
        ...
        if not result:
            logging.warning(
                f"Warning in get_kode_pos: No unique postal code records found"
            )
            return (
                jsonify(
                    {
        ...
        # Mengembalikan response
        logging.info(
            f"Success in get_kode_pos: Unique postal code records retrieved successfully"
        )
        return (
            jsonify(
                {
